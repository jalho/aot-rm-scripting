// ANNOTATED MEDITERRANEAN from C:\Program Files (x86)\Microsoft Games\Age of Mythology\docs\aom random map help file.rtf

// Main entry point for random map script. All scripts need to void out all previous commands, so always start with a void.

void main(void)
{

  // Text. The status text is useful only in displaying the progress bar. You can also provide information inside the quotations, such as “generating seas.”

   rmSetStatusText("",0.01);

   // Set size. This map defaults to placing 7500 tiles per player. If the map is generated as a large map by the user, then it uses 9750 tiles instead. The integer for size is just converting tiles to square meters and then reporting how large the map is.
   ...
   // Init map. We want the base water type to be Mediterranean Sea, but want the base texture that is placed on the map to be not water, but land, using grassDirt25. If nothing else was done in this script, we would have a large square of grassDirt25.
   rmSetSeaType("mediterranean sea");
   rmTerrainInitialize("GrassDirt25");

   // Define some classes. Most of these classes will be used later for constraints, but keeping them all in one location will make them easier to find later. Some are defined as integers, as a shortcut for those cases where we reference the classes a lot, but classes can be done either way.
